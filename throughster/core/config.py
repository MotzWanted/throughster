...
class ClientSettings(pydantic_settings.BaseSettings):
    ...
    model_config = pydantic_settings.SettingsConfigDict(
        extra="ignore", protected_namespaces=("settings_",)
    )

    API_BASE: str = pydantic.Field(
        ..., description="API host, e.g., `http://localhost:8080`"
    )
    API_KEY: str = pydantic.Field(..., description="API key")
    MODEL_NAME: str | None = pydantic.Field(
        None, description="The name or identifier of the deployed model."
    )
    CACHE_DIR: Path = pydantic.Field(
        Path("~/.cache/llm-client").expanduser(), description="Cache directory"
    )
...
class AzureClientSettings(ClientSettings):
    ...
    API_VERSION: str | None = pydantic.Field(..., description="API version")


class OpenAiClientSettings(ClientSettings):
